...
unsigned int str2hex(char *param)
{
    char *addr = param;
    unsigned int value = 0;

    if ('0' != addr[0] || 'x' != addr[1])
    {
        printf("lgy_dbg : argv 2 should be 0x........ ERROR!!!\n");
        return 0;
    }

    for (int i = 0; i < 8; i++)
    {
        if ((addr[2 + i] >= '0') && (addr[2 + i] <= '9'))
        {
            addr[2 + i] = addr[2 + i] - '0';
        }
        else if ((addr[2 + i] >= 'a') && (addr[2 + i] <= 'f'))
        {
            addr[2 + i] = addr[2 + i] - 'a' + 10;
        }
        else if ((addr[2 + i] >= 'A') && (addr[2 + i] <= 'F'))
        {
            addr[2 + i] = addr[2 + i] - 'A' + 10;
...
void doRegRead(int argc, char *argv [])
{
    char *addr = argv[2];
    ...
    if (argc < 3)
    {
        printf("lgy_dbg : arc = %d ERROR!!!\n", argc);
        return;
    }

    printf("lgy_dbg : arc = %d\n", argc);

    regaddr = str2hex(addr);

    printf("lgy_dbg : regaddr = 0x%08x\n", regaddr);
    regvalue = readl(regaddr);
    printf("lgy_dbg : regvalue = 0x%08x\n", regvalue);

    return;
}

void doRegWrite(int argc, char *argv [])
{
    char *addr = argv[2];
    char *value = argv[3];
    unsigned int regaddr = 0;
    ...
    regaddr = str2hex(addr);
    regvalue = str2hex(value);

    printf("lgy_dbg : regaddr = 0x%08x\n", regaddr);
    printf("lgy_dbg : regvalue = 0x%08x\n", regvalue);
    printf("lgy_dbg : writing...\n");
    writel(regvalue, regaddr);
...
// Core wiringPi functions
  ...
  else if (strcasecmp (argv [1], "regread") == 0) doRegRead   (argc, argv) ;
  else if (strcasecmp (argv [1], "regwrite") == 0) doRegWrite (argc, argv) ;
